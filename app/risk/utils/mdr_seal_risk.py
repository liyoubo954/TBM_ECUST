...
def map_project_data(data: Dict[str, Any]) -> Dict[str, float]:
    mapped_data = {
        'P0': data.get('WorkCham.Pres.01', 0),       # 工作仓压力1#
        'P1': data.get('MB.InSeal.Grs.Pres', 0),    # 油气密封反馈压力
        'P2': data.get('MD.TelSeal.Grs.Pres.01', 0),    # 1#主驱动伸缩密封油脂压力
        'P3': data.get('GOil.OilGasSeal.Pres', 0),     # 齿轮油油气密封压力
        'P4': data.get('OilGasSeal.LeakDetCham.Pres', 0)  # 油气密封泄露检测腔压力
    }
