...
--influxdb.database cjtl_dg1213_ring
...
### run flink的顺序
1. 在服务器路径`/root/home/flink_tysj`上启动服务 `docker compose up -d` 或者重启服务 `docker compose restart`
2. 在ringclean_cjtl项目中`mvn clean package` 打jar包
3. 上传ringclean_cjtl的jar包到`http://172.16.105.238:8081/#/overview`
4. 获取ringclean_cjtl的jar包id`curl http://localhost:8081/jars`
